...
namespace DataTierGeneratorPlusLibrary 
{
	class SqlGenerator: DbGenerator 
	{
		...
		) 
		{
            ...
            try
            {
                if (model.GrantUser.Length > 0)
                {
                    String fileName;

                    // Determine the file name to be used
                    if (model.CreateMultipleFiles)
                    {
                    ...
                    {
                        writer.Write(GeneratorModel.DatabaseUtility.GetUserQueries(databaseName, model.GrantUser));
                    }
                }
		...
		) 
		{
            ...
            try
            {
                ...
                String procedureName = model.SPPrefix + table.Name + "Insert";
                String fileName;
                ...
                using (StreamWriter writer = new StreamWriter(fileName, true))
                {
                    ...
                    for (int i = 0; i < table.Columns.Count; i++)
                    {
                        ...
                        if (!column.IsRowGuidCol && !column.IsIdentity)
                        {
                            ...
                            {
                                writer.WriteLine("\t" + GeneratorModel.DatabaseUtility.CreateStoredProcedureFormalParameterString(column) + ",");
                            }
                            else
                            {
                                writer.WriteLine("\t" + GeneratorModel.DatabaseUtility.CreateStoredProcedureFormalParameterString(column));
                    ...
                    foreach (Column column in table.Columns)
                    {
                        ...
                        {
                            writer.WriteLine("DECLARE " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias) + " uniqueidentifier");
                            writer.WriteLine("SET " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias) + " = NEWID()");
                            writer.WriteLine();
                            break;
                        }
                    }
                    ...
                    for (int i = 0; i < table.Columns.Count; i++)
                    {
                        ...
                        if (!column.IsIdentity)
                        {
                            ...
                            if (i < (table.Columns.Count - 1))
                            {
                                writer.WriteLine("\t" + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias) + ",");
                            }
                            else
                            {
                                writer.WriteLine("\t" + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias));
                    ...
                    foreach (Column column in table.Columns)
                    {
                        ...
                        {
                            writer.WriteLine();
                            writer.WriteLine("SELECT " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.Name) + " AS " + column.Name);
                            break;
                        }
                    }
                    ...
                    {
                        writer.WriteLine();
                        writer.WriteLine("GRANT EXECUTE ON [" + table.Schema + "].[" + procedureName + "] TO [" + model.GrantUser + "]");
                        writer.WriteLine("GO");
                    }
                }
	    ...
	    ) 
		{
            ...
            try
            {
                ...
                {
                    // Create the stored procedure name
                    String procedureName = model.SPPrefix + table.Name + "Update";
                    String fileName;
                    ...
                    using (StreamWriter writer = new StreamWriter(fileName, true))
                    {
                        ...
                        for (int i = 0; i < table.Columns.Count; i++)
                        {
                            ...
                            if (!table.PrimaryKeys.Contains(column))
                            {
                                if (i < (table.Columns.Count - 1))
                                {
                                    writer.WriteLine("\t[" + column.Name + "] = " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias) + ",");
                                }
                                else
                                {
                                    writer.WriteLine("\t[" + column.Name + "] = " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias));
                        ...
                        for (int i = 0; i < table.PrimaryKeys.Count; i++)
                        {
                            ...
                            if (i > 0)
                            {
                                writer.Write("\tAND [" + column.Name + "] = " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias));
                            }
                            else
                            {
                                writer.Write("\t [" + column.Name + "] = " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias));
                            }
                        }
                        ...
                        //							writer.WriteLine("\t"); 
                        //						} 
                        //						writer.WriteLine("(COALESCE([" + column.Name + "], " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias) + ") IS NULL " +  
                        //							"OR [" + column.Name + "] = " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias) + ")"); 
                        //					}
		...
		) 
		{
            ...
            try
            {
                ...
                {
                    // Create the stored procedure name
                    String procedureName = model.SPPrefix + table.Name + "Delete";
                    String fileName;
                    ...
                    using (StreamWriter writer = new StreamWriter(fileName, true))
                    {
                        ...
                        for (int i = 0; i < table.PrimaryKeys.Count; i++)
                        {
                            ...
                            if (i > 0)
                            {
                                writer.WriteLine("\tAND [" + column.Name + "] = " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias));
                            }
                            else
                            {
                                writer.WriteLine("\t[" + column.Name + "] = " + GeneratorModel.DatabaseUtility.FormatStoredProcedureParameterName(column.ProgrammaticAlias));
		...
		) 
		{
            ...
            {//List<Column> foreignKeys = (List<Column>)table.ForeignKeys[fkName];
                ...
                foreach (List<Column> compositeKeyList in table.ForeignKeys.Values)
                {
                    ...
                    stringBuilder.Append(model.SPPrefix + table.Name + "DeleteBy");
		...
		) 
		{
            ...
            try
            {
                ...
                {
                    // Create the stored procedure name
                    String procedureName = model.SPPrefix + table.Name + "Select";
                    String fileName;
		...
		) 
		{
            ...
            try
            {
                ...
                {
                    // Create the stored procedure name
                    String procedureName = model.SPPrefix + table.Name + "SelectAll";
                    String fileName;
		...
		) 
		{
            ...
            try
            {
                ...
                foreach (List<Column> compositeKeyList in table.ForeignKeys.Values)
                {
                    // Create the stored procedure name
                    StringBuilder stringBuilder = new StringBuilder(255);
                    stringBuilder.Append(model.SPPrefix + table.Name + "SelectBy");
