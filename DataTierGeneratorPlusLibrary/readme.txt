DataTierGeneratorPlusLibrary v6.15
...
Note: This tool has been made largely obsolete by the combination of the Entity Framework (EF) and WCF Data Services (OData). See the MVC3 project also posted on http://www.ssepan.com.
...
In the binary version, they are generated under <special_folder>\DataTierGeneratorPlusLibrary\<database>\<[sql|language]>, where the special folder is currently LocalApplicationData (which maps to <OS_drive>:\users\<user>\AppData\Local). 
...
~Fixed no handler wired up to Settings in forms. Renamed 'PropertyChangedEventHandlerDelegate' to 'ModelPropertyChangedEventHandlerDelegate'. Wired up latter in 'InitViewModel' after model handler in view. Currently handling 'Dirty' property.
...
~Modified Settings to implement part of its interfaces as a new interface ISettingsComponent.

6.14: 
~Using Ssepan.* v2.5
~Fixed progress bar and status message logic.
~Moved Model and Controller (and Settings counterparts) to MVC code folder.
~Moved MVC and all generation-related code/content to a new DTGP Library.
~Moved load and generate methods to controller.
~Converted table and column ArrayLists to List<Table>, List<Column>.
~Converted foreign hey HashTable to Dictionary<String, List<Column>>
~Fixed error getting type System.Binary for sql type 'image' and others; return Byte[] or Object instead.
...
~Due to permissions issues in Vista/Windows7, changed app to generate files under user's special application local folders (<drive>:\Users\<profile>\AppData\Local\DataTierGeneratorPlusLibrary\). Since this location is not easily found, the application was also changed to open the Explorer window to that location on a successful generation.
